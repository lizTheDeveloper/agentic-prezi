...
- **#2 research engine is BUILT** (`src/research/*`; full pipeline scope→discover→rank→synthesize→**grounding ⚑**→validate, emitting the `#3 §4` contract; OpenAlex/Crossref/arXiv/PubMed adapters; budget caps + HTTP hardening; 138 tests; verified against live APIs; plan in `docs/superpowers/plans/2026-06-08-sub2-research-engine.md`). Run with `npm run research -- "<write-up>"`. **The real `llm` is now wired into `runResearch({ llm })`** via the Nous Portal provider (`src/research/providers/nous.mjs`, OpenAI-compatible `/v1/chat/completions`, Bearer `NOUS_RESEARCH_API_KEY`, default model `anthropic/claude-opus-4.8`; override with `NOUS_RESEARCH_MODEL` / `NOUS_RESEARCH_BASE_URL`). Verified live end-to-end (LLM scope + synthesis → grounded, DOI-resolved findings). **The LLM is REQUIRED — there is no deterministic fallback:** a missing key or a provider failure errors out loudly (the scope/synthesize heuristics were removed). **Still TODO: the Hermes/cloud-browser extraction adapter** (the §8 full-text enrichment path).

**Next:** **#4 deploy/hosting** (Hetzner + Docker, wildcard TLS, gVisor sandbox) + the **Hermes drivability spike**; then add the Hermes/cloud-browser **extraction adapter** (§8 enrichment); then **#3 presentation generator**. Seam for #2→#1: `runResearch()` in `src/research/pipeline.mjs` feeds the generator that currently produces the #1 stub.
