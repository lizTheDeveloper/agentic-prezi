...
**Routing (changed 2026-06-08):** published presentations are now served **path-based on ONE dedicated host** — `presentations.themultiverse.school/p/<slug>/` (config `PUBLISHED_HOST`, default `presentations.<baseDomain>`) — **not** per-slug subdomains. The app is on `presapp.themultiverse.school` (cookies; config `APP_HOST` — `app.` is taken by a sibling service on the shared box). Two explicit single-level subdomains, **no wildcard** (a wildcard would capture the ~40 sibling `*.themultiverse.school` deployments on the shared box). Origin isolation preserved (published host is cookieless, strict CSP). `src/published.ts` has both `handlePublishedPath` (path-based, the deploy) and legacy host-based `handlePublished`.

**Auth = The Multiverse School SSO (2026-06-09, replaced magic-link):** Aethrix authenticates via the platform's shared `.themultiverse.school` session — read the `session` cookie, resolve `school:session:{sid}` from the School's Redis (same instance `bazaar`/`campus` use, on the `coolify` Docker network), honor `expires`, take `data.email`, upsert a local user. Contract reverse-engineered from `bazaar`'s `school-session.js`. Zero-dep: `src/redis-min.ts` (RESP AUTH+GET over `node:net`, length-prefixed = injection-safe) + `src/school-sso.ts` (fail-closed resolver, short per-sid cache so School logout propagates). No Aethrix session cookie; the School cookie is the source of truth. Magic-link + dev-login **removed**; logout → School logout; unauthenticated SPA bounces to `SCHOOL_LOGIN_URL?next=`. Prod guard: **refuses to start without `SCHOOL_SESSION_REDIS_URL`** (no fallback auth). Any valid School login is admitted. Email (`src/email-sendgrid.ts`) is now optional (notifications only). **Live + verified end-to-end** (throwaway Redis blob → `/api/me` returns the user through Cloudflare). 192 tests.

**#4 deploy — LIVE (2026-06-09):** Aethrix (#1 skeleton) is deployed and publicly reachable as a **Coolify application** on the main box, behind Coolify's Traefik + Cloudflare (orange-cloud, Universal SSL, Full mode). **App:** `https://presapp.themultiverse.school` (SPA + API). **Published decks:** `https://presentations.themultiverse.school/p/<slug>/`. Prod mode verified: `/api/dev/login` 404s, published host rejects SPA/API/unknown-slugs, SendGrid email accepted, persistent volume mounted at `/app/data` (survives redeploys). Coolify dashboard: `http://<origin-IP: local memory only>:8000` (admin `<admin email: local memory>`). Coolify env, domains, and deploys are driven via its API (token on the box); the persistent volume was added directly to Coolify's `local_persistent_volumes` table because the API doesn't expose storage in beta.463. **Still deferred:** the gVisor sandbox-broker, Hermes worker, Playwright vision loop, and self-hosted GlitchTip stack (the stub generator needs none) — and Cloudflare TLS mode / origin-cert hardening review.

**#4 deploy — earlier skeleton notes (2026-06-08):** the target is the operator's **main** box `the main box` (<origin-IP: local memory only>), which is **Coolify-managed** — `coolify-proxy` (Traefik v3.6) owns :80/:443 and DNS is on **Cloudflare**. So the self-Caddy + DNS-01-wildcard + build-on-box-tarball design in spec §4/§7 **does not apply to this box**; we deploy as a **Coolify application** and let Traefik+Cloudflare own routing/TLS. Built + locally verified (prod-mode boot serves 200, `/api/dev/login` 404s): `Dockerfile` (node:26, zero-dep, `node src/server.ts`), `.dockerignore`, `deploy/prod.env.example`, `deploy/coolify-deploy.md` (architecture + go-live sequence). **Three operator-held blockers before public launch:** (1) Cloudflare DNS records for `app.`/`*.themultiverse.school`; (2) a transactional **email provider** — prod magic-link is the only auth (`DEV_AUTH_BYPASS` refused in prod) and `src/email.ts` has only `ConsoleEmailSender`, so **no login is possible until it's wired**; (3) Coolify admin access to create the app. Wildcard TLS likely needs **no** DNS-01 (Cloudflare Universal SSL covers one-level `*.themultiverse.school` at the edge — verify the account plan). gVisor/Hermes-worker/Playwright/GlitchTip stack deferred (stub needs none).
